			...
			outerloop: while (n < 60) {
				for (Appointment appointment : appointmentsUsed) {
					if (appointment.getStartTimeHour() == i && appointment.getStartTimeMinute() == n) {
						...
						case SELECTED:
							resultSelected.add(new Appointment(workHours.getId(), workHours.getSpecialist(), appointment.getUser(), appointment.getSlotState(), 1, 1, 2016, appointment.getStartTimeHour(), appointment.getStartTimeMinute(), i, n));
							break outerloop;
						case CONFIRM:
							resultConfirm.add(new Appointment(workHours.getId(), workHours.getSpecialist(), appointment.getUser(), appointment.getSlotState(), 1, 1, 2016, appointment.getStartTimeHour(), appointment.getStartTimeMinute(), i, n));
							break outerloop;
						case CLOSED:
							resultClosed.add(new Appointment(workHours.getId(), workHours.getSpecialist(), appointment.getUser(), appointment.getSlotState(), 1, 1, 2016, appointment.getStartTimeHour(), appointment.getStartTimeMinute(), i, n));
							break outerloop;
						default:
							break outerloop;
